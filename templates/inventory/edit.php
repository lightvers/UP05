...
$page_title = "Редактирование инвентаризации";
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            ...
            $inventory->status = $form_data['status'];

            if ($inventory->update()) {
                $_SESSION['success'] = 'Инвентаризация успешно обновлена';
                header('Location: view.php?id='.$id);
                exit;
            } else {
                throw new Exception('Ошибка при обновлении инвентаризации');
            }
        }
    }
...
<div class="content-header">
    <h1 class="content-title">Редактирование инвентаризации</h1>
    <div>
        ...
        <form method="POST" id="inventoryForm">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="status" name="status">
                    <option value="planned" <?= $form_data['status'] === 'planned' ? 'selected' : '' ?>>Запланирована</option>
                    <option value="in_progress" <?= $form_data['status'] === 'in_progress' ? 'selected' : '' ?>>В процессе</option>
                    <option value="completed" <?= $form_data['status'] === 'completed' ? 'selected' : '' ?>>Завершена</option>
                </select>
            </div>
